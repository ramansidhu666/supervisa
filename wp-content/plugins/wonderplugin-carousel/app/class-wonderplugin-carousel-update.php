...
class WonderPlugin_Carousel_Update {
	...
	function __construct($controller) {
		...
		$this->slug = 'wonderplugin-carousel';
		$this->plugin = WONDERPLUGIN_CAROUSEL_PLUGIN;
		$this->version = WONDERPLUGIN_CAROUSEL_PLUGIN_VERSION;
	...
	function plugin_update_notice() {
		...
		if( $pagenow === 'update-core.php')
		{
			if ( !isset($info->key_status) || $info->key_status != 'valid')
			{
				...
				{
					if (WONDERPLUGIN_CAROUSEL_VERSION_TYPE == 'F')
					{
					...
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin, please <a href="'. admin_url('admin.php?page=wonderplugin_carousel_register') . '">enter your license key</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';
					}
				}
			}
		}
	...
	function show_plugin_update_message( $plugin_data, $r ) {
		...
		else
		{
			...
			{
				echo '<br>To get automatic update, please <a href="'. admin_url('admin.php?page=wonderplugin_carousel_register') . '">enter your license key</a> or <a href="https://www.wonderplugin.com/register-faq/#manualupdate" target="_blank">update the plugin manually</a>.';
			}
		}
	}
}