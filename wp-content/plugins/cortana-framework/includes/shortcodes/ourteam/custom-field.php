<div class="my_meta_control">
	...
	<?php while ( $mb->have_fields_and_multi( 'ourteam', array( 'length' => 2 ) ) ): ?>
		...
		<label><?php _e( 'Name', 'cortana' ); ?></label>
		<input class="form-control" type="text" name="<?php esc_attr( $mb->the_name() ); ?>"
			   value="<?php esc_attr( $mb->the_value() ); ?>" />

		<?php $mb->the_field( 'socialLink' ); ?>
		<label><?php _e( 'Link', 'cortana' ); ?></label>
		<input class="form-control" type="text" name="<?php esc_attr( $mb->the_name() ); ?>"
			   value="<?php esc_attr( $mb->the_value() ); ?>" />

		<?php $mb->the_field( 'socialIcon' ); ?>
		<label><?php _e( 'Icon', 'cortana' ); ?></label>
		<input class="form-control input-icon" style="width:62%;" type="text" name="<?php esc_attr( $mb->the_name() ); ?>"
			   value="<?php esc_attr( $mb->the_value() ); ?>" />
		<input title="Click to browse icon" style="width:35%;" class="browse-icon button-secondary" type="button"
			   value="Browse Icon">
	...
	<p>
		<a href="#" class="docopy-ourteam button"><?php _e( 'Add Social', 'cortana' ); ?></a>
	</p>
</div>
<style>
