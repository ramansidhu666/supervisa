...
if ( !current_user_can( 'edit_posts' ) ) {
	wp_die( __( 'You do not have sufficient permissions to access this page.', 'cortana' ) );
}
...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST[$post_type . '-slug'] ) ) {
	...
	if ( $_POST[$post_type . '-slug'] != '' || $_POST['category-' . $post_type . '-slug'] != '' ) {
		...
		if ( !$is_match_slug || !$is_match_taxonomy_slug ) {
			update_option( 'g5plus-cortana-' . $post_type . '-config', $config );
	...
	<div class="page-post-type-setting">
		...
		<ul>
			...
			<?php if ( $taxonomy_slug != '' ) { ?>
				...
				<li class="title">New slug post type:</li>
				<li class="value">
					<input type="text" name="<?php echo esc_attr( 'category-' . $post_type . '-slug' ) ?>" /></li>
				...
				<li class="value">
					<input type="text" name="<?php echo esc_attr( 'category-' . $post_type . '-name' ) ?>" /></li>
			<?php } ?>

			<li class="title"></li>
			<li class="value"><input class="button button-large button-primary" type="submit" value="Save Changes" />
			</li>
		</ul>
	</div>
</form>
