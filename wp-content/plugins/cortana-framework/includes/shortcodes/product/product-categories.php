...
if ( !class_exists( 'g5plusFramework_Shortcode_Product_Categories' ) ) {
	class g5plusFramework_Shortcode_Product_Categories {
		function __construct() {
			add_shortcode( 'cortana_product_categories', array( $this, 'product_categories_shortcode' ) );
		}

		function  product_categories_shortcode( $atts ) {
			$title_style = $title = $category = $columns = $slider = $hide_empty = $orderby = $order = $el_class = $css_animation = $duration = $delay = '';
			...
			<?php if ( $product_categories ) : ?>
				<div class="<?php echo esc_attr( $class_name ) ?>" <?php echo g5plusFramework_Shortcodes::g5plus_get_style_animation( $duration, $delay ); ?>>
					<?php if ( !empty( $title ) ) : ?>
						<h3 class="cortana-title <?php echo esc_attr( $title_class ) ?>">
							<span><?php echo esc_html( $title ); ?></span></h3>
					<?php endif; ?>
