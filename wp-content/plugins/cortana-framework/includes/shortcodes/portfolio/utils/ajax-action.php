...
function g5plusframework_portfolio_load_by_category() {
	...
	$short_code = sprintf( '[g5plusframework_portfolio category="%s" column="%s" item="%s" show_pagging="2" padding="%s" current_page="%s" order="%s"]', $category, $column, $posts_per_page, $padding, $current_page, $order );
	echo do_shortcode( $short_code );
	die();
}