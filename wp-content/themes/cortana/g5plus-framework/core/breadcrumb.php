...
if ( !function_exists( 'g5plus_get_breadcrumb' ) ) {
	function g5plus_get_breadcrumb() {
		...
		$breadcrumbs = '<ul class="breadcrumbs ' . esc_attr( $class_breadcrumbs ) . '">';
...
if ( !function_exists( 'g5plus_get_breadcrumb_items' ) ) {
	function g5plus_get_breadcrumb_items() {
		...
		$item = array();
		/* Front page. */
		$item[] = '<li class="first" typeof="v:Breadcrumb">' . esc_html__( 'YOU ARE HERE: ', 'cortana' ) . '</li>';
		if ( is_front_page() ) {
			$item['last'] = esc_html__( 'Home', 'cortana' );
		}


		/* Link to front page. */
		if ( !is_front_page() ) {
			$item[] = '<li class="home" typeof="v:Breadcrumb"><a rel="v:url" property="v:title" href="' . home_url( '/' ) . '" class="home">' . esc_html__( 'Home', 'cortana' ) . '</a></li>';
		}
		...
		elseif ( is_singular() ) {
			...
			if ( is_archive() ) {
				...
				if ( is_category() || is_tag() || is_tax() ) {
					...
					if ( function_exists( 'is_post_type_archive' ) && is_post_type_archive() ) {
						...
						if ( is_date() ) {
							...
							if ( is_day() ) {
								$item['last'] = esc_html__( 'Archives for ', 'cortana' ) . get_the_time( 'F j, Y' );
							} elseif ( is_month() ) {
								$item['last'] = esc_html__( 'Archives for ', 'cortana' ) . single_month_title( ' ', false );
							} elseif ( is_year() ) {
								$item['last'] = esc_html__( 'Archives for ', 'cortana' ) . get_the_time( 'Y' );
							}
						} else {
							if ( is_author() ) {
								$item['last'] = esc_html__( 'Archives by: ', 'cortana' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
							}
						}
					}
				}
				...
				if ( is_search() ) {
					$item['last'] = esc_html__( 'Search results for "', 'cortana' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';
				} /* If viewing a 404 error page. */
				else {
					if ( is_404() ) {
						$item['last'] = esc_html__( 'Page Not Found', 'cortana' );
					}
				}
			}
		}
