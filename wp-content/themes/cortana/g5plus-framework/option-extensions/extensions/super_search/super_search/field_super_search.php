    ...
    class ReduxFramework_super_search extends ReduxFramework {
        ...
        function __construct( $field = array(), $value ='', $parent ) {
            ...
            if ( empty( $this->extension_dir ) ) {
                $this->extension_dir = trailingslashit( str_replace( '\\', '/', get_template_directory()  . '/g5plus-framework/option-extensions/extensions/super_search/super_search' ) );
       ...
       public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => __( 'Filter', 'cortana' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="redux-supersearch-accordion" data-new-content-title="' . esc_attr ( sprintf ( __( 'New %s', 'cortana' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    //BEFORE TEXT
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'beforetext' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'beforetext' ] ) : __( 'Before Text', 'cortana' );
                    echo '<li><input type="text" id="' . $this->field[ 'id' ] . '-before_text_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][beforetext]' . $this->field['name_suffix'] . '" value="' . esc_attr ( $slide[ 'beforetext' ] ) . '" placeholder="' . $placeholder . '" class="full-text slide-title" /></li>';
              		...
              		//FILTER 
			        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'ss-filter' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'ss-filter' ] ) : __( 'Filter', 'cortana' );
					...
					echo '<option value="">'.__( 'Choose an option', 'cortana' ).'</option>';
					...
					//LABEL
			        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'label' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'label' ] ) : __( 'Label', 'cortana' );
					...
					//AFTER TEXT
			        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'aftertext' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'aftertext' ] ) : __( 'After Text', 'cortana' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-supersearch-remove">' . sprintf ( __( 'Delete %s', 'cortana' ), $this->field[ 'content_title' ] ) . '</a></li>';
			...
			/* Label Field */
				...
				/*   After Text Field */
				$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'after' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'after' ] ) : __( 'After Text', 'cortana' );
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-supersearch-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __( 'Add %s', 'cortana' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
