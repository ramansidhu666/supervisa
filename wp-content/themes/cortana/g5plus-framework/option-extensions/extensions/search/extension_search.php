    ...
    class ReduxFramework_extension_search {
      ...
      public function __construct( $parent ) {
        ...
        if (empty($this->extension_dir)) {
            $this->_extension_dir = trailingslashit(str_replace('\\', '/', get_template_directory()  . '/g5plus-framework/option-extensions/extensions/search'));
            $this->_extension_url = site_url(str_replace(trailingslashit(str_replace('\\', '/', ABSPATH)), '', $this->_extension_dir));

            // Customize: hoantv
            $this->_extension_url = apply_filters('redux/_extension_url',$this->_extension_url);
        }    
