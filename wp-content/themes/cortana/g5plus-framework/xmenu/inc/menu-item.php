	...
	<div class="xmenu-config-panel-wrapper">
		<div class="xmenu-header">
			<h2>
				...
				<button class="x-button xmenu-config-panel-save" type="button"><i class="fa fa-save"></i> <?php echo esc_html__('Save Changes','cortana') ?></button>
		...
		<div class="xmenu-config-panel-left">
			<ul>
				...
				<?php endforeach; ?>
				<li class="x-reset">
					<i class="fa fa-refresh"></i> <?php echo esc_html__('Reset','cortana') ?>
				</li>
			</ul>
		</div>
		...
		<div class="xmenu-icon-popup">
			<div class="xmenu-icon-popup-header">
				<input type="text" placeholder="<?php echo esc_html__('Type to search...','cortana') ?>"/>
...
function xmenu_save_config_callback() {
	$error_result = array(
		'code' => '-1',
		'message' => esc_html__('Save menu config fail','cortana'),
	);
	$config = $_POST['config'];
	$menu_id = $_POST['id'];
	...
	if ( $id && ! is_wp_error( $id ) ) {
		...
		update_post_meta( (int) $menu_id, '_menu_item_xmenu_config', maybe_unserialize( $config ) );
