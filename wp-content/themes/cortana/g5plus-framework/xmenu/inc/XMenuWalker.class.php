...
class XMenuWalker extends Walker_Nav_Menu {
	...
	function custom_style_menu_item($item_id, $menu_data, $setting_options){
		...
		//add style
		if (!empty($item_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . '{' . $item_style . '}';
		}
		if (!empty($item_text_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' > a{' . $item_text_style . '}';
		}
		if (!empty($item_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ':hover,'
		...
		if (!empty($item_text_style_active)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' > a:hover,'
				. '#menu-item-' . $item_id . '.current-menu-item > a,'
				. '#menu-item-' . $item_id . '.current-menu-ancestor > a' . '{' . $item_text_style_active . '}';
		}
		if (!empty($icon_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' .x-menu-icon' .'{' . $icon_style . '}';
		}
		if (!empty($submenu_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' > ul.x-sub-menu' .'{' . $submenu_style . '}';
		}
		if (!empty($submenu_style_active)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ':hover > ul.x-sub-menu,'
				. '#menu-item-' . $item_id . '.current-menu-item > ul.x-sub-menu,'
				. '#menu-item-' . $item_id . '.current-menu-ancestor > ul.x-sub-menu'
				.'{' . $submenu_style_active . '}';
		}
		if (!empty($item_feature_text_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' span.x-menu-feature' .'{' . $item_feature_text_style . '}';
		}
		if (!empty($item_feature_text_style_after)) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' span.x-menu-feature:after' .'{' . $item_feature_text_style_after . '}';
		}

		if (isset($menu_data['submenu-col-spacing-default']) && !empty($menu_data['submenu-col-spacing-default'])) {
			$this->xmenu_custom_style .= '#menu-item-' . $item_id . ' > ul.x-sub-menu > li+li' .'{padding-left:'. $menu_data['submenu-col-spacing-default'] . 'px}';
		}
	}
	...
	function print_custom_style() {
		...
		//echo '<style id="xmenu-custom-style">' . $this->xmenu_custom_style .  '</style>';
	}
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ($depth > 0) {
			...
			if (isset($xmenu_meta['widget-area']) && $xmenu_meta['widget-area']) {
				$widgets_arr = get_option(XMENU_MENU_ITEM_WIDGET_AREAS, array());
				if (isset($widgets_arr[$item->post_name])) {
					if (is_active_sidebar(XMENU_MENU_WIDGET_AREAS_ID . $item->post_name)) {
						ob_start();
						dynamic_sidebar( XMENU_MENU_WIDGET_AREAS_ID . $item->post_name );
