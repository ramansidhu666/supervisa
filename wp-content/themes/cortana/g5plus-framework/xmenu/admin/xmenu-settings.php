...
if ( !class_exists('XMenu_Setting_Options' ) ):
	class XMenu_Setting_Options {
		...
		function __construct() {
			...
			$this->setting_options = array(
				'settings' => array(
					...
					'config' => array(
						'setting-heading' => array(
							'text' => esc_html__('Settings','cortana'),
							'type' => 'heading',
						),

						/*'setting-responsive-breakpoint' => array(
							'text' => __('Responsive Breakpoint','cortana'),
							'type' => 'text',
							'std' => '',
						...
						'transition' => array(
							'text' => esc_html__('Select Menu Transition','cortana'),
							'type' => 'select',
							'std' => 'none',
							'options' => xmenu_get_transition()
						),
						'transition-duration' => array(
							'text' => __('Transition Duration','cortana'),
							'type' => 'text',
							'std' => '',
				...
				'images' => array(
					...
					'config' => array(
						'image-heading' => array(
							'text' => esc_html__('Images Settings','cortana'),
							'type' => 'heading',
						),
						'image-size' => array(
		...
		function admin_menu() {
			add_theme_page(
				esc_html__('XMENU Settings','cortana'),
				esc_html__('XMENU Settings','cortana'),
				'manage_options',
				'xmenu-settings',
		...
		function plugin_page() {
			...
			<div class="xmenu-settings">
				<h2>
					<a class="tab <?php echo ($current_menu == '' ? 'active' : '') ?>" href="themes.php?page=xmenu-settings"><strong><?php echo esc_html__('XMENU','cortana') ?></strong> <?php echo esc_html__('Setting','cortana') ?></a>
					<?php foreach ($setting_menus as $setting_menu): ?>
						<a class="tab <?php echo ($current_menu == $setting_menu ? 'active' : '') ?>" href="<?php echo esc_url('themes.php?page=xmenu-settings&menu=' . $setting_menu) ?>"><strong><?php echo ($setting_menu); ?></strong> <?php echo esc_html__('Setting','cortana') ?></a>
					<?php endforeach; ?>
					<a class="tab" href="themes.php?page=xmenu-settings&do=widget"><strong><?php echo esc_html__('XMENU','cortana') ?></strong> <?php esc_html__('Widget Manager','cortana') ?></a>
					<button id="setting-add-menu" type="button" class="button button-primary x-fr"><?php echo esc_html__('Add Menu Setting','cortana') ?></button>
				</h2>
				<form method="post" action="themes.php?page=xmenu-settings" novalidate="novalidate" id="xmenu_settings">
					...
					<p class="submit">
						...
						<button type="button" id="xmenu-save-setting" class="button button-primary"><i class="fa fa-save"></i> <?php echo esc_html__('Save Changes','cortana')?></button>
						<?php if (!empty($current_menu)): ?>
							<input id="xmenu_menu_slug" type="hidden" name="menu_slug" value="<?php echo esc_attr($current_menu) ?>"/>
							<button type="button" class="button button-primary" id="xmenu-delete-setting"><i class="fa fa-remove"></i> <?php echo esc_html__('Delete Settings','cortana')?></button>
						<?php endif;?>
					</p>
				</form>
				<div class="x-popup" id="xmenu-setting-popup">
					<div class="x-popup-inner">
						<h3><?php echo esc_html__('Select Menu for your new settings','cortana') ?> <i id="setting-add-menu-close" class="fa fa-close"></i></h3>
						<div class="x-popup-select">
							...
							<button id="xmenu-create-button" type="button"><?php echo esc_html__('Create Menu Setting','cortana') ?></button>
						</div>
					</div>
				</div>
			</div>
		...
		function widget_manager() {
			...
			<div class="xmenu-settings">
				<h2>
					<a class="tab" href="themes.php?page=xmenu-settings"><strong><?php echo esc_html__('XMENU','cortana') ?></strong> <?php esc_html__('Setting','cortana') ?></a>
					<?php foreach ($setting_menus as $setting_menu): ?>
						<a class="tab" href="<?php echo esc_url('themes.php?page=xmenu-settings&menu=' . $setting_menu)  ?>"><strong><?php echo esc_html($setting_menu); ?></strong> <?php echo esc_html__('Setting','cortana') ?></a>
					<?php endforeach; ?>
					<a class="tab active" href="themes.php?page=xmenu-settings&do=widget"><strong><?php echo esc_html__('XMENU','cortana') ?></strong> <?php echo  esc_html__('Widget Manager','cortana') ?></a>
				</h2>
						...
						<tbody>
							<?php foreach( $widgets as $menu_item_id => $widget_title ):?>
								<tr>
									...
									<td><?php echo esc_html($widget_title) . esc_html__(' [Menu Item ', 'cortana') . esc_html($menu_item_id) .']'; ?></td>
								</tr>
							<?php endforeach;?>
						</tbody>
					</table>
					<p><?php echo esc_html__('Choose which Custom Widget Areas to delete', 'cortana') ?></p>
					<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary" value="<?php echo esc_html__('Delete Widget Areas','cortana')?>"></p>
				</form>
			</div>
			<?php
		}
		...
		function generate_css_file($option_key) {
			require_once get_template_directory() . '/g5plus-framework/xmenu/inc/generate-less/Less.php';
			$setting_default = $this->get_setting_defaults();
		...
		function xmenu_setting_create() {
			$error_result = array(
				'code' => '-1',
				'message' => esc_html__('Create menu settings fail','cortana'),
			);
		...
		function xmenu_delete_setting() {
			$error_result = array(
				'code' => '-1',
				'message' => esc_html__('Delete menu settings fail','cortana'),
			);
