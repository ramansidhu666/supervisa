...
if ( !defined( 'WP_LOAD_IMPORTERS' ) ) {
	var_dump( __( 'Not define WP_LOAD_IMPORTERS', 'cortana' ) );

	return;
}

if ( !defined( 'G5PLUS_THEME_MOD_NAME' ) ) {
	define( 'G5PLUS_THEME_MOD_NAME', 'cortana' );
}

define( 'CHANGE_DATA_FILE', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'change-data.json' );
...
require ( get_template_directory()  . '/g5plus-framework/install-demo/parsers.php');
	...
	class G5_Import extends WP_Importer {
		...
		function import( $file ) {
			...
			if ( file_exists( MENU_MAPPING ) ) {
				@$this->processed_menu_items = json_decode( $wp_filesystem->get_contents( MENU_MAPPING ), true );
			}
			if ( file_exists( PROCESS_TERM ) ) {
				@$this->processed_terms = json_decode( $wp_filesystem->get_contents( PROCESS_TERM ), true );
			}
			if ( file_exists( PROCESS_POSTS ) ) {
				@$this->processed_posts = json_decode( $wp_filesystem->get_contents( PROCESS_POSTS ), true );
			}
			if ( file_exists( MENU_ITEM_ORPHANS ) ) {
				@$this->menu_item_orphans = json_decode( $wp_filesystem->get_contents( MENU_ITEM_ORPHANS ), true );
			}
			if ( file_exists( MENU_MISSING ) ) {
				@$this->missing_menu_items = json_decode( $wp_filesystem->get_contents( MENU_MISSING ), true );
			}
			if ( file_exists( URL_REMAP ) ) {
				@$this->url_remap = json_decode( $wp_filesystem->get_contents( URL_REMAP ), true );
			}
			if ( file_exists( POST_ORPHANS ) ) {
				@$this->post_orphans = json_decode( $wp_filesystem->get_contents( POST_ORPHANS ), true );
			}
			if ( file_exists( FEATURE_IMAGES ) ) {
				@$this->featured_images = json_decode( $wp_filesystem->get_contents( FEATURE_IMAGES ), true );
			}
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( !empty( $this->authors ) ) : ?>
					<h3><?php echo esc_html__( 'Assign Authors', 'wordpress-importer' ); ?></h3>
					<p><?php echo esc_html__( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'wordpress-importer' ); ?></p>
				...
				<?php if ( $this->allow_fetch_attachments() ) : ?>
					<h3><?php echo esc_html__( 'Import Attachments', 'wordpress-importer' ); ?></h3>
					<p>
						<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
						<label for="import-attachments"><?php echo esc_html__( 'Download and import file attachments', 'wordpress-importer' ); ?></label>
					</p>
				<?php endif; ?>

				<p class="submit">
					<input type="submit" class="button" value="<?php echo esc_html__( 'Submit', 'wordpress-importer' ); ?>" />
				</p>
			</form>
			<?php
		}
		...
		function author_select( $n, $author ) {
			echo esc_html__( 'Import author:', 'wordpress-importer' );
			...
			if ( $create_users ) {
				if ( $this->version != '1.0' ) {
					echo esc_html__( 'or create new user with login name:', 'wordpress-importer' );
					$value = '';
				} else {
					echo esc_html__( 'as a new user:', 'wordpress-importer' );
			...
			if ( !$create_users && $this->version == '1.0' ) {
				echo esc_html__( 'assign posts to an existing user:', 'wordpress-importer' );
			} else {
				echo esc_html__( 'or assign posts to an existing user:', 'wordpress-importer' );
			}
		...
		function process_menu_item( $item ) {
			...
			if ( !$menu_slug ) {
				echo esc_html__( 'Menu item skipped due to missing menu slug', 'wordpress-importer' );
				echo '<br />';
		...
		function fetch_remote_file( $url, $post ) {
			if ( $_REQUEST['method'] == 'no-get-image' ) {
				$demo_site = 'http://' . G5PLUS_THEME_MOD_NAME . '.osthemes.biz/wp-content/uploads';

				$upload_info = wp_upload_dir( null );

				$return_file = str_replace( $demo_site, $upload_info['basedir'], $url );
				$return_url  = str_replace( $demo_site, $upload_info['baseurl'], $url );
...
function g5plus_wordpress_importer_init() {
	...
	register_importer( 'wordpress', 'WordPress', __( 'Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'wordpress-importer' ), array( $GLOBALS['g5_import'], 'dispatch' ) );
}
