...
class g5plus_import extends G5_Import {
	...
	function import_revslider( $other_data ) {
		$is_import = false;
		if ( $handle = opendir( trailingslashit( get_template_directory() ) . "assets" . DIRECTORY_SEPARATOR . "data-demo" . DIRECTORY_SEPARATOR . "revslider" ) ) {
			...
			while ( false !== ( $entry = readdir( $handle ) ) ) {
				...
				if ( $entry != "." && $entry != ".." ) {
					$rev_import_file = trailingslashit( get_template_directory() ) . "assets" . DIRECTORY_SEPARATOR . "data-demo" . DIRECTORY_SEPARATOR . "revslider" . DIRECTORY_SEPARATOR . $entry;
